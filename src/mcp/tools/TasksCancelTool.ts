  ...
  async execute(_input: TasksCancelInput): Promise<TasksCancelResponse> {
    ...
    try {
      if (task.status === "backlog" || task.status === "todo") {
        this._rules.assertForbiddenFields(task, ["started_at", "done_at", "tool"]);
        this._rules.assertForbiddenFields(task, ["canceled_at"]);
    ...
    const canceledAt = this._clock.nowIsoWithOffset();
    const updated = this._transitions.toCanceled(task, { canceledAt });

    try {
      await this._taskStore.write(updated);
      await this._git.commitAll(`cancel ${updated.id}`);
    } catch {
      return {
        ok: false,
