...
* **Компоненты**:
    ...
    * `ProjectRegistry` — обнаружение проектов как подпапок `repoRoot` + нормализация имени проекта (см. ограничения на имена проектов).
...
* Проекты = подпапки внутри `repoRoot`. Создаются вручную (через создание папки).
...
* **US-2: Получить список проектов**
    ...
    * AC:
        ...
        * Given в `repoRoot` есть подпапки проектов, When `projects.list`, Then возвращаются только проекты с именами, проходящими `^[a-z0-9-]+$`.
...
* **Релиз**: подключение MCP сервера локально, настройка `repoRoot`.
