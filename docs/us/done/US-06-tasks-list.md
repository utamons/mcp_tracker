...
#### Описание
- Реализовать инструмент `tasks.list(project, status?, type?, text?)`.
- Возвращать список задач проекта; при `status` — только задачи указанного статуса; при `type` — только задачи указанного типа; при `text` — задачи, где `text` найден в `title` и/или body.
...
- Классы/модули:
  ...
  - `TaskQueryService` (`src/tasks/TaskQueryService.ts`) — методы: `list(project: string, filter: { status?: TaskStatus; type?: TaskType; text?: string }): Promise<TaskView[]>`
...
- `[TasksList_filterByType — с type возвращает только задачи этого типа.]`
