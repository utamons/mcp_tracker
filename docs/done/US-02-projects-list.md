...
#### Описание
- Реализовать инструмент `projects.list`, который сканирует `repoRoot` и возвращает список подпапок проектов.
...
#### Дизайн
- Классы/модули:
  - `Config` (`src/config/Config.ts`) — методы: `getRepoRoot(): string`
...
- `[ProjectsListTool_ioError — при ошибке чтения repoRoot возвращает IO_ERROR и error.message на английском.]`
